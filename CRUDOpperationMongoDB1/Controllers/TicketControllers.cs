...
namespace TicketAPI.Controllers
{
    ...
    public class TicketController : ControllerBase
    {
        ...
        {
            var ticket = await _ticketService.GetTicketByIdAsync(id); // Gọi service để tìm ticket theo ID
        ...
        public async Task<IActionResult> ExportTicketsToExcel()
        {
            ...
            var tickets = await _ticketService.FindTickets(_ => true); // Lấy toàn bộ ticket từ MongoDB
        ...
        public async Task<IActionResult> ExportTicketsToExcel([FromQuery] string? fromAddress, [FromQuery] string? toAddress)
        {
            ...
            var tickets = await _ticketService.FindTickets(filter);
        ...
        public async Task<IActionResult> ExportTicketsToExcel([FromQuery] int page = 1, [FromQuery] int pageSize = 25)
        {
            ...
            var tickets = await _ticketService.FindTickets(_ => true);
        ...
        public async Task<IActionResult> CreateTicket([FromBody] CreateTicketDTO ticketDto)
        {
            ...
            try
            {
                ...
                var ticketResult = await _ticketService.InsertOneTicketAsync(tickets);
                // trả về kq vừa tạo
        ...
        public async Task<IActionResult> UpdateTicket(string id, [FromBody] CreateTicketDTO toDTO)
        {
            ...
            await _ticketService.UpdateTicketAsync(id, TicketMapper.EntityToUpdateTicketDTO(existingTicket));
            return Ok(TicketMapper.EntityToUpdateTicketDTO(existingTicket)); // Trả về thông tin vé sau khi cập nhật
        }
        [HttpPost("import")]
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    for (int row = 2; row <= rowCount; row++) // Bỏ qua dòng tiêu đề
                    {
                        ...
                        var status = worksheet.Cells[row, 8].Text.Trim();
                        var customerId = worksheet.Cells[row, 9].Text.Trim();
                        ...
                        {
                            existingTicket = await _ticketService.GetTicketByIdAsync(ticketId);
                        }
                        ...
                        if (existingTicket != null) // Vé đã tồn tại, cập nhật thông tin
                        {
                            ...
                            existingTicket.CustomerId = customerId;
                            // B1: Thêm customerId và xóa customer phone name trong file excel import
                            //b2: Kiểm tra customerId có tồn tại trong hệ thống không nếu  
                            // + có: thì lấy được  customerName;  customerPhone;
                            // + không : thì continue
                            ...
                            existingTicket.CustomerName = customer.CustomerName;
                            existingTicket.CustomerPhone = customer.CustomerPhone;
                            await _ticketService.UpdateTicketAsync(ticketId, TicketMapper.EntityToUpdateTicketDTO(existingTicket));
                        }
                        ...
                        else // Vé chưa tồn tại, tạo mới
                        {
                            ...
                            await _ticketService.InsertOneTicketAsync(newTicket);
                        }
                    }
                }
            }
        ...
        {
            var ticket = await _ticketService.GetTicketByIdAsync(id); // Lấy vé theo ID
            if (ticket == null) return NotFound("Vé không tồn tại!"); // Nếu không tìm thấy, trả về lỗi 404

            // Cập nhật trạng thái sang "Huỷ"
            await _ticketService.UpdateTicketStatusAsync(id, "Huỷ");
